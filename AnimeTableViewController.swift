...
class AnimeTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let castedCell = cell as? MyTableViewCell {
            castedCell.fillCell(with: titles[indexPath.row])
        }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if let vc = storyboard?.instantiateViewController(withIdentifier: "1") as? ViewController {
                vc.data = titles[indexPath.row]
